...
public class EjecucionExcel4 {
    ...
    public void ModificarElementos() {
        for (int numeroFila = 0; numeroFila < numeroFilas; numeroFila++) {
            ...
            try {
                AbrirNavegador();            // Abre navegador
                instanciasBasicas();         // Crea instancias necesarias
                lecturaDatos(numeroFila);   // Lee los datos desde Excel

                esc_Registrof.RegistroFacebook(ent_registro); // Ejecuta escenario principal

                // Si salió bien
                dat_excel.EscrituraDatos(numeroFila, "resultado", "OK");

            } catch (Exception e) {
                System.err.println("Error: " + e.getMessage());

                // Si falló algo, escribir "ERROR" en la columna resultado
                dat_excel.EscrituraDatos(numeroFila, "resultado", "ERROR");

            } finally {
                CerrarNavegador(); // Siempre cerrar el navegador, ocurra o no una excepción
            }
        }
    }
}
